...
bool TextFormatInit() {
  if (!pTextFormatKaiTi) {
    pWriteFactory->CreateTextFormat(L"KaiTi", NULL, DWRITE_FONT_WEIGHT_MEDIUM,
  ...
  if (!pTextFormatSimSun) {
    pWriteFactory->CreateTextFormat(L"SimSun", NULL, DWRITE_FONT_WEIGHT_MEDIUM,
                                    DWRITE_FONT_STYLE_NORMAL,
  ...
  if (!pTextFormatHealth) {
    pWriteFactory->CreateTextFormat(L"Msyh", NULL, DWRITE_FONT_WEIGHT_MEDIUM,
                                    DWRITE_FONT_STYLE_NORMAL,
                                    DWRITE_FONT_STRETCH_NORMAL,  // Stretch
                                    12.0f,                       // Size
                                    L"MyFont",                   // Local
...
bool SolidColorBrushInit() {
  if (!DrawBrush) {
    pRender->CreateSolidColorBrush(D2D1::ColorF(D2D1::ColorF::Black),
                                   &DrawBrush);
    return true;
  }
  return false;
...
bool LinearGradientBrushInit(int X, int Y, int W, int H, D3DCOLOR Color,
                             D3DCOLOR Color2, float Alpha) {
  SAFE_RELEASE(DrawGradientBrush);
  if (!DrawGradientBrush) {
    ...
    pRender->CreateGradientStopCollection(GRADIENT_STOP, 2, D2D1_GAMMA_2_2,
                                          D2D1_EXTEND_MODE_CLAMP,
                                          &GradientStopCollection);
    pRender->CreateLinearGradientBrush(
        D2D1::LinearGradientBrushProperties(D2D1::Point2F((float)X, (float)Y),
                                            D2D1::Point2F(float(W), float(H))),
        GradientStopCollection, &DrawGradientBrush);
    return true;
  }
  return false;
}

void DrawRenderText(int X, int Y, const char* text, D3DCOLOR Color,
                    IDWriteTextFormat* TextFormat, float Alpha) {
  WCHAR WStr[256] = {0};
  ...
  pRender->DrawText(WStr, wcslen(WStr), TextFormat,
                    D2D1::RectF((float)X, (float)Y, (float)MyRect.right,
                                (float)MyRect.bottom),
                    DrawBrush);
}
void DrawVectorLine(D3DXVECTOR3 X, D3DXVECTOR3 Y, D3DCOLOR Color, float Width,
                    float Alpha) {
  DrawBrush->SetColor(D2D1::ColorF(Color, Alpha));
  pRender->DrawLine(D2D1::Point2F(X.x, X.y), D2D1::Point2F(Y.x, Y.y), DrawBrush,
                    Width);
}
void DrawLine(int X, int Y, int XX, int YY, D3DCOLOR Color, float Width,
              float Alpha) {
  DrawBrush->SetColor(D2D1::ColorF(Color, Alpha));
  pRender->DrawLine(D2D1::Point2F(X, Y), D2D1::Point2F(XX, YY), DrawBrush,
                    Width);
}
void DrawFillRectangle(int X, int Y, int W, int H, D3DCOLOR Color,
                       float Alpha) {
  DrawBrush->SetColor(D2D1::ColorF(Color, Alpha));
  pRender->FillRectangle(
      D2D1::RectF((float)X, (float)Y, float(X + W), float(Y + H)), DrawBrush);
}
void DrawFillRectangleGradient(int X, int Y, int W, int H, D3DCOLOR Color,
                               D3DCOLOR Color2, float Alpha = 1.0f) {
  LinearGradientBrushInit(X, Y, W, H, Color, Color2, Alpha);
  pRender->FillRectangle(
      D2D1::RectF((float)X, (float)Y, float(X + W), float(Y + H)),
      DrawGradientBrush);
}
void DrawRectangle(float X, float Y, float W, float H, D3DCOLOR Color,
                   float Width, float Alpha) {
  DrawBrush->SetColor(D2D1::ColorF(Color, Alpha));
  pRender->DrawRectangle(D2D1::RectF(X, Y, X + W, Y + H), DrawBrush, Width);
}
void DrawRectangleGradient(float X, float Y, int W, int H, D3DCOLOR Color,
                           D3DCOLOR Color2, float Width = 1.0f,
                           float Alpha = 1.0f) {
  LinearGradientBrushInit(X, Y, W, H, Color, Color2, Alpha);
  pRender->DrawRectangle(D2D1::RectF(X, Y, W, H), DrawGradientBrush, Width);
}
void DrawFillRoundedRectangle(int X, int Y, int W, int H, float RadianX,
                              float RadianY, D3DCOLOR Color,
                              float Alpha = 1.0f) {
  DrawBrush->SetColor(D2D1::ColorF(Color, Alpha));
  pRender->FillRoundedRectangle(
      D2D1::RoundedRect(
          D2D1::RectF((float)X, (float)Y, float(X + W), float(Y + H)), RadianX,
          RadianY),
      DrawBrush);
}
void DrawFillRoundedRectangleGradient(int X, int Y, int W, int H, float RadianX,
                                      float RadianY, D3DCOLOR Color,
                                      D3DCOLOR Color2, float Alpha = 1.0f) {
  LinearGradientBrushInit(X, Y, W, H, Color, Color2, Alpha);
  pRender->FillRoundedRectangle(
...
void DrawRoundedRectangle(int X, int Y, int W, int H, float RadianX,
                          float RadianY, D3DCOLOR Color, float Width = 1.0f,
                          float Alpha = 1.0f) {
  DrawBrush->SetColor(D2D1::ColorF(Color, Alpha));
  pRender->DrawRoundedRectangle(
      D2D1::RoundedRect(D2D1::RectF((float)X, (float)Y, float(W), float(H)),
                        RadianX, RadianY),
      DrawBrush, Width);
}
void DrawRoundedRectangleGradient(int X, int Y, int W, int H, float RadianX,
                                  float RadianY, D3DCOLOR Color,
                                  D3DCOLOR Color2, float Width = 1.0f,
                                  float Alpha = 1.0f) {
  LinearGradientBrushInit(X, Y, W, H, Color, Color2, Alpha);
  pRender->DrawRoundedRectangle(
      D2D1::RoundedRect(D2D1::RectF((float)X, (float)Y, float(W), float(H)),
                        RadianX, RadianY),
      DrawGradientBrush, Width);
}
void DrawFillRound(float X, float Y, float Radius, D3DCOLOR Color,
                   float Alpha = 1.0f) {
  DrawBrush->SetColor(D2D1::ColorF(Color, Alpha));
  pRender->FillEllipse(D2D1::Ellipse(D2D1::Point2F(X, Y), Radius, Radius),
                       DrawBrush);
}
void DrawRound(float X, float Y, float Radius, D3DCOLOR Color,
               float Width = 1.0f, float Alpha = 1.0f) {
  DrawBrush->SetColor(D2D1::ColorF(Color, Alpha));
  pRender->DrawEllipse(D2D1::Ellipse(D2D1::Point2F(X, Y), Radius, Radius),
                       DrawBrush, Width);
}
void DrawEllipse(float X, float Y, float RadiusX, float RadiusY, D3DCOLOR Color,
                 float Width, float Alpha) {
  DrawBrush->SetColor(D2D1::ColorF(Color, Alpha));
  pRender->DrawEllipse(D2D1::Ellipse(D2D1::Point2F(X, Y), RadiusX, RadiusY),
                       DrawBrush, Width);
}
void DrawFillEllipse(float X, float Y, float RadiusX, float RadiusY,
                     D3DCOLOR Color, float Alpha = 1.0f) {
  DrawBrush->SetColor(D2D1::ColorF(Color, Alpha));
  pRender->FillEllipse(D2D1::Ellipse(D2D1::Point2F(X, Y), RadiusX, RadiusY),
                       DrawBrush);
}
...
void DrawHalfRectangle(float X, float Y, float W, float H, D3DCOLOR Color,
                       float Width, float Alpha)  //��ʽ����
{
  float DivW = W / 4.5;
  float DivH = H / 3.5;
...
void DrawVerticalHealth(float Health, float MaxHealth, int X, int Y, int W,
                        int H)  //��Ҫ����
{
  if (H >= -20) H = -21;
  ...
  DrawFillRoundedRectangle(X, Y - 1, W, H, 3, 1,
                           D3DCOLOR_RGBA(0, 0, 0, 255));  
  DrawFillRoundedRectangle(X + 1, Y, W, h, 3, 1, color, 0.8); 

  // 	char text[20];
  // 	sprintf_s(text, "HP: %d", (INT)Health);
  // 	DrawRenderText(X-10, Y, text, color, pTextFormatHealth);
}
void DrawTransverseHealth(float Health, float MaxHealth, float _X, int X, int Y,
                          int W, int H) {
  if (W <= 30) W = 31;
  int progress = (int)((float)Health / ((float)MaxHealth / 100));
  int w = (int)((float)W / 100 * progress);
  ...
  DrawFillRoundedRectangle(X, Y - 1, W, H, 3, 1,
                           D3DCOLOR_RGBA(0, 0, 0, 255));  
  DrawFillRoundedRectangle(X + 1, Y, w, H, 3, 1, color);  
...
void MyChack::MyCreateChack(int X, int Y, int W, int H, D3DCOLOR Color,
                            const char* Text) {
  MyChackX = X;
  MyChackY = Y;
  MyChackW = W;
...
void MyMenu::MyCreateMenu(POINT MenuPoint, int W, int H, D3DCOLOR Color,
                          const char* Text) {
  MyMenuX = MenuPoint.x;
  MyMenuY = MenuPoint.y;
  MyMenuW = W;
  ...
  if (MyMenuHideShow) {
    ...
    DrawLine(MyMenuX + 1, MyMenuY + 25, MyMenuX + 1, MyMenuY + H + 12,
             Color);  //����
    DrawLine(MyMenuX + W - 1, MyMenuY + 25, MyMenuX + W - 1, MyMenuY + H + 12,
             Color);  //����
    DrawLine(MyMenuX, MyMenuY + H + 13, MyMenuX + W, MyMenuY + H + 13, Color);
    DrawFillRectangle(MyMenuX + 1, MyMenuY + 13, W - 2, H, D2D1::ColorF::Black,
                      0.6f);
    DrawRenderText(MyMenuX + 13, MyMenuY, Text, D2D1::ColorF::Black,
                   pTextFormatMsyh);
    DrawRenderText(MyMenuX + 15, MyMenuY - 2, Text, D2D1::ColorF::White,
                   pTextFormatMsyh);
  }
}
...
void MyMenu::MyClickPoint(POINT MousePoint) {
  ...
  if (ClickMouse) {
    if (!DragMenu &&
        ((MyMenuX < MousePoint.x) && ((MyMenuX + MyMenuW) > MousePoint.x)) &&
...
void MyRadio::MyCreateRadio(int X, int Y, int W, int H, D3DCOLOR Color,
                            const char* Text) {
  MyRadioX = X;
  MyRadioY = Y;
  MyRadioW = W;
  ...
  DrawRenderText(X + 4, Y + (H / 5) + 2, Text, D2D1::ColorF::Black,
                 pTextFormatSimSun);
  DrawRenderText(X + 6, Y + (H / 5), Text, D2D1::ColorF::White,
                 pTextFormatSimSun);
}
void MyHealthButton::MyCreateButton(int X, int Y, int W, int H,
                                    D3DCOLOR Color) {
  MyButtonX = X + 6;
  MyButtonY = Y + (H / 2);
  MyButtonW = W;
  ...
  DrawLine(MyButtonX + 11, MyButtonY + 1, MyButtonX + 3, MyButtonY + 8,
           D2D1::ColorF::White, 2.5);
  DrawLine(MyButtonX + 3, MyButtonY + 8, MyButtonX + 11, MyButtonY + 14,
           D2D1::ColorF::White, 2.5);
  DrawFillRectangle(MyButtonX + 50, MyButtonY, W, H, Color);
  DrawLine(MyButtonX + 53, MyButtonY + 1, MyButtonX + 62, MyButtonY + 8,
           D2D1::ColorF::White, 3);
...
void MyHealthButton::MyButtonClick(POINT MousePoint) {
  bool ClickMouse = (GetAsyncKeyState(VK_LBUTTON) & 0x8000) ? 1 : 0;
  if (ClickMouse && ((MyButtonX < MousePoint.x) &&
                     ((MyButtonX + MyButtonW) > MousePoint.x)) &&
      ((MyButtonY < MousePoint.y) &&
       ((MyButtonH + MyButtonY) > MousePoint.y))) {
    MyButtonSelect = !MyButtonSelect;
    Sleep(100);
  }
  if (ClickMouse && ((MyButtonX + 50 < MousePoint.x) &&
                     ((MyButtonX + 50 + MyButtonW) > MousePoint.x)) &&
      ((MyButtonY < MousePoint.y) &&
       ((MyButtonH + MyButtonY) > MousePoint.y))) {
    MyButtonSelect = !MyButtonSelect;
    Sleep(100);
...
void MyAimBotHeight::MyCreateButton(int X, int Y, int W, int H,
                                    D3DCOLOR Color) {
  MyButtonX = X + 6;
  MyButtonY = Y + (H / 2);
  MyButtonW = W;
  MyButtonH = H;
  DrawFillRectangle(MyButtonX, MyButtonY, W, H, Color);
  DrawLine(MyButtonX + MyButtonW / 2, MyButtonY + 1, MyButtonX + 2,
           MyButtonY + 12, D2D1::ColorF::White, 2.5);
  DrawLine(MyButtonX + MyButtonW / 2, MyButtonY + 1, MyButtonX + MyButtonW - 2,
           MyButtonY + 12, D2D1::ColorF::White, 2.5);
  DrawFillRectangle(MyButtonX, MyButtonY + 50, W, H, Color);
  DrawLine(MyButtonX + MyButtonW / 2, MyButtonY + 64, MyButtonX + 2,
...
void MyAimBotHeight::MyButtonClick(POINT MousePoint) {
  ...
  if (ClickMouse && ((MyButtonX < MousePoint.x) &&
                     ((MyButtonX + MyButtonW) > MousePoint.x)) &&
      ((MyButtonY + 50 < MousePoint.y) &&
       ((MyButtonH + MyButtonY + 50) > MousePoint.y))) {
    MyHeightInt -= 1;
    Sleep(100);
  }
}
...
void MyChangeButton::MyCreateButton(int X, int Y, int W, int H, D3DCOLOR Color,
                                    char* Text) {
  MyButtonX = X + 6;
  MyButtonY = Y + (H / 2);
  MyButtonW = W;
  ...
  DrawRenderText(MyButtonX - 83, MyButtonY + 19, Text, D2D1::ColorF::White,
                 pTextFormatSimSun);
  DrawRenderText(MyButtonX + 13, MyButtonY + 20, A, D2D1::ColorF::White,
                 pTextFormatSimSun);
}
