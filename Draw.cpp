...
bool LinearGradientBrushInit(int X, int Y, int W, int H, D3DCOLOR Color, D3DCOLOR Color2, float Alpha)
{
	SAFE_RELEASE(DrawGradientBrush);
...
void DrawRenderText(int X, int Y, const char* text, D3DCOLOR Color, IDWriteTextFormat*TextFormat,float Alpha)
{
	WCHAR WStr[256] = { 0 };
...
void DrawVectorLine(D3DXVECTOR3 X, D3DXVECTOR3 Y, D3DCOLOR Color, float Width  , float Alpha )
{
	DrawBrush->SetColor(D2D1::ColorF(Color, Alpha));
	pRender->DrawLine(D2D1::Point2F(X.x, X.y), D2D1::Point2F(Y.x, Y.y), DrawBrush, Width);
}
void DrawLine(int X, int Y, int XX, int YY, D3DCOLOR Color, float Width , float Alpha)
{
	DrawBrush->SetColor(D2D1::ColorF(Color, Alpha));
	pRender->DrawLine(D2D1::Point2F(X, Y), D2D1::Point2F(XX, YY), DrawBrush, Width);
}
void DrawFillRectangle(int X, int Y, int W, int H ,D3DCOLOR Color, float Alpha)
{
	DrawBrush->SetColor(D2D1::ColorF(Color, Alpha));
	pRender->FillRectangle(D2D1::RectF((float)X, (float)Y, float(X + W), float(Y + H)), DrawBrush);
}
void DrawFillRectangleGradient(int X, int Y, int W, int H, D3DCOLOR Color, D3DCOLOR Color2, float Alpha = 1.0f)
{
...
void DrawRectangle(float X, float Y, float W, float H, D3DCOLOR Color, float Width, float Alpha )
{
	DrawBrush->SetColor(D2D1::ColorF(Color, Alpha));
	pRender->DrawRectangle(D2D1::RectF(X, Y, X + W, Y + H), DrawBrush, Width);
}
void DrawRectangleGradient(float X, float Y, int W, int H, D3DCOLOR Color, D3DCOLOR Color2, float Width = 1.0f, float Alpha = 1.0f)
{
	LinearGradientBrushInit(X, Y, W, H, Color, Color2, Alpha);
	pRender->DrawRectangle(D2D1::RectF(X, Y, W, H), DrawGradientBrush, Width);
}
void DrawFillRoundedRectangle(int X, int Y, int W, int H, float RadianX, float RadianY, D3DCOLOR Color, float Alpha = 1.0f)
{
	DrawBrush->SetColor(D2D1::ColorF(Color, Alpha));
	pRender->FillRoundedRectangle(D2D1::RoundedRect(D2D1::RectF((float)X, (float)Y, float(X + W), float(Y + H)),
		RadianX, RadianY), DrawBrush);
}
void DrawFillRoundedRectangleGradient(int X, int Y, int W, int H, float RadianX, float RadianY, D3DCOLOR Color, D3DCOLOR Color2,float Alpha = 1.0f)
{
	LinearGradientBrushInit(X, Y, W, H, Color, Color2, Alpha);
	pRender->FillRoundedRectangle(D2D1::RoundedRect(D2D1::RectF((float)X, (float)Y, float(X + W), float(Y + H)),
		RadianX, RadianY), DrawGradientBrush);
}
void DrawRoundedRectangle(int X, int Y, int W, int H, float RadianX, float RadianY, D3DCOLOR Color, float Width = 1.0f, float Alpha = 1.0f)
{
	DrawBrush->SetColor(D2D1::ColorF(Color, Alpha));
	pRender->DrawRoundedRectangle(D2D1::RoundedRect(D2D1::RectF((float)X, (float)Y, float(W), float(H)), RadianX, RadianY), DrawBrush, Width);
}
void DrawRoundedRectangleGradient(int X, int Y, int W, int H, float RadianX, float RadianY, D3DCOLOR Color, D3DCOLOR Color2, float Width = 1.0f, float Alpha = 1.0f)
{
	LinearGradientBrushInit(X, Y, W, H, Color, Color2, Alpha);
...
void DrawFillRound(float X, float Y, float Radius, D3DCOLOR Color, float Alpha = 1.0f)
{
	DrawBrush->SetColor(D2D1::ColorF(Color, Alpha));
	pRender->FillEllipse(D2D1::Ellipse(D2D1::Point2F(X, Y), Radius, Radius), DrawBrush);
}
void DrawRound(float X, float Y, float Radius, D3DCOLOR Color, float Width = 1.0f, float Alpha = 1.0f)
{
	DrawBrush->SetColor(D2D1::ColorF(Color, Alpha));
	pRender->DrawEllipse(D2D1::Ellipse(D2D1::Point2F(X, Y), Radius, Radius), DrawBrush, Width);
}
void DrawEllipse(float X, float Y, float RadiusX, float RadiusY, D3DCOLOR Color, float Width, float Alpha )
{
	DrawBrush->SetColor(D2D1::ColorF(Color, Alpha));
	pRender->DrawEllipse(D2D1::Ellipse(D2D1::Point2F(X, Y), RadiusX, RadiusY), DrawBrush, Width);
}
void DrawFillEllipse(float X, float Y, float RadiusX, float RadiusY, D3DCOLOR Color, float Alpha = 1.0f)
{
	DrawBrush->SetColor(D2D1::ColorF(Color, Alpha));
